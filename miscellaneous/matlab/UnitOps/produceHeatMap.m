function [ HeatMap ] = produceHeatMap( x,y,z )
%This function produces a graph of corresponding date, hour and third
%property 

 %create a grid with one-meter resolution
 cellsize=1;
 minx=min(x);
 maxx=max(x);
 miny=min(y);
 ...
 for k=1:length(zind)
     ...
     Z(zind(k,1),zind(k,2))=mean(z(ind));
 ...
 ylabel('Time')
 c1=colorbar;
 set(get(c1,'ylabel'),'string',input(prompt))
